...
func TestRunner_Execute(t *testing.T) {
	t.Run("should execute a plan", func(t *testing.T) {
		...
		m1 := newMockMigration(s.ctrl, "1")
		m2 := newMockMigration(s.ctrl, "2")
	...
	t.Run("should execute a plan with reporter", func(t *testing.T) {
		...
		reporter.EXPECT().BeforeExecute(ActionTypeDo, m1).Return()
		reporter.EXPECT().AfterExecute(ActionTypeDo, m1, nil).Return()

		reporter.EXPECT().BeforeExecute(ActionTypeUndo, m2).Return()
		reporter.EXPECT().AfterExecute(ActionTypeUndo, m2, nil).Return()
